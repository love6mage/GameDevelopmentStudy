...
- `Terrain` 地形，有时在像 Maya 这样的包中明确地建模，但是如果玩家可以从很远的距离看到，通常需要动态曲面细分或者其他 LOD 系统，同时可能还需要限制呈现非常大的户外区域所需的数据
...
- `Water` 水渲染器现今在游戏中十分常见。有许多不同类型的水，例如海洋、水池、河流、瀑布、喷泉、射流、水坑、潮湿的固体表面等，通常每种类型都需要专门的渲染技术。有些还需要动态运动模拟。大水体可能需要类似于地形系统使用的动态曲面细分或其他 LOD 技术
  - 水系统有时会和游戏中的刚体动力学系统（漂浮、水射流的力量等）和游戏玩法（滑溜的表面、游泳力学、潜水力学、骑垂直水射流等）交互
  - 水效果通常结合不同的渲染技术和子系统创建，例如，一个瀑布可能使用专门的水渲染器、滚动纹理、底部雾气的粒子效果、泡沫的贴花状覆盖层等。

### Overlays

- 大多数游戏有抬头显示器（`heads-up displays` / `HUD`）、游戏中的图形用户界面（`graphical user interfaces` / `GUI`）和菜单系统，这些覆盖层包含二维或三维图像，直接在观察空间或屏幕空间渲染。覆盖层通常在主场景后渲染，禁用深度测试，确保他们在 3D 场景之上显示。二维覆盖层一般在屏幕空间使用正交投影渲染四边形实现；三维覆盖层可以用正交投影或常规透视投影渲染，其中几何图形位于观察空间
- `Normalized Screen Coordinates` 标准化屏幕坐标，二维覆盖层可以通过屏幕像素计量，但是由于不同屏幕分辨率和宽高比的存在，最好使用标准化屏幕坐标。最简单的方式是定义标准化屏幕坐标，使得 y 轴坐标范围为 $[0,1]$，这样在宽高比为 $4:3$ 时 x 轴坐标范围为 $[0,4/3]$，宽高比为 $16:9$ 时 x 轴坐标范围为 $[0,16/9]$。不能同时定义 x 轴和 y 轴坐标范围为 $[0,1]$，这样可能会使覆盖层在不同宽高比时产生不同伸缩，不可接受
- `Relative Screen Coordinates` 相对屏幕坐标，通过使用相对屏幕坐标，宽高比变化时不需要变化与屏幕相对的标准化坐标。与可视元素相对的标准化坐标受宽高比影响，需要使用多种可以独立调整的不同的布局，每种布局对应一种宽高比
- `Text and Fonts` 游戏的文字字体系统通常作为一种特殊的二维或三维覆盖层实现。文字渲染系统需要有能力显示一个文本字符串对应的一系列字符字形，在屏幕上以各种方向排列。字体通常通过包含不同字形的纹理贴图实现。一个字符描述文件提供了纹理中每个字形的边界框和字体布局信息如字距、基线偏移量等
  - 一个好的文字字体系统必须考虑到各种语言固有的字符集和阅读方向的差异
  - 有些字体系统提供了各种有趣的特性，如每个字符的动画，甚至有些引擎实现了 Adobe Flash 标准的一个子集来支持丰富的二维效果
- `Gamma Correction` 伽马校正，CRT 显示器往往对亮度值有非线性响应，当线性 RGB 值发送到 CRT 时，屏幕上的结果图像对于人眼感官是非线性的。
  - 典型 CRT 显示器的伽马响应曲线可以用公式简单建模：$V_{out}=V_{in}^\gamma\ ,\ \gamma_{CRT}>1$
  - 为了校正这个效果，颜色在发送给 CRT 显示器前一般先通过一个逆转换，即使用一个伽马值 $\gamma_{corr}$。典型 CRT 显示器的伽马值为 $\gamma_{CRT}=2.2$，所以校正值为 $\gamma_{corr}\approx1/2.2=0.455$
  - 伽马编码由 3D 渲染引擎执行以确保最终图像的值是正确伽马校正的。一个问题是用于表示纹理贴图的位图通常本身就是伽马校正的。高质量的渲染引擎在渲染前对纹理进行伽马解码，然后重新伽马编码最终渲染场景以便其颜色可以在屏幕上正确再现
