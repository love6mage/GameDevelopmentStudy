...
- 底层流水线相当于 OpenGL 或 DirectX 用于动画，功能强大，但游戏代码直接使用相当不方便。因此，在底层流水线和游戏角色以及动画系统的其他客户端之间通常引入一个中间层，这层通常实现为状态机，称为动作状态机（`action state machine`）或动画状态机（`animation state machine`）
...
- 大多数活的生物可以用它们的身体同时做多件事情，且身体不同部分的运动通常不完全同步，一些部分往往“引导”其他部分的运动，例如头部引导转向，接着是肩膀、臀部，最后是腿部。在传统动画中，这种众所周知的技术称为预期（`anticipation`）
- 上述运动与基于状态机的动画方法不一致，解决方法是引入状态层（`state layers`）的概念，每层一次只能处于一个状态，但这些层在时间上彼此独立。骨架最终姿势的计算方法为：评估 n 层中每个层上的混合树，从而生成 n 个骨架姿势，然后以预定义的方式混合这些姿势
  - 顽皮狗引擎使用了一个分层状态架构，这些层形成一个栈，最底层总是产生一个全身骨架姿势，每个更上层都在基础姿势上混合一个新的全身、部分骨架或加性姿势。支持 LERP 和加性两种类型的层

### Control Parameters

- 从软件工程的角度看，编排复杂动画角色的所有混合权重、回放速率和其他控制参数可能具有挑战性
- 挑战
  - 扁平的加权平均架构中，我们有可能在角色上播放的所有动画片段的扁平表，每个片段状态有混合权重、回放速率和可能的其他控制参数。控制角色的代码必须通过名称查找每个片段状态并适当调整它们的混合权重，这使接口变得简单，但是将控制混合权重的大部分责任转移到了角色控制系统。这样细粒度的方式会很乏味，而且会导致源码难以理解
  - 混合树架构中存在不同的问题：控制参数埋在树中，控制角色的代码必须提前了解混合树结构才能找到合适节点并控制其参数
- 解决方法
  - `Node search` 一些引擎为更上层代码提供在树中查找混合节点的方法，例如树中的相关节点可以给出特殊的名称，控制代码可以简单地在树中搜索特定名称的节点
  - `Named variables` 一些引擎允许为各个控制参数分配名称，控制代码可以按名称查找控制参数，以便调整其值
  - `Control structure` 其他引擎中，一个简单的数据结构（例如浮点值数组或 C 结构体）包含整个角色的所有控制参数，混合树中的节点可以通过两种方式连接到特定控制参数：硬编码以使用某些结构体成员；按名称或索引查找参数

### Constraints

- 角色动画控制的另一个重要方面是以各种方式约束场景中角色和物体的运动，例如约束一个武器让它似乎总是在携带它的角色手中，约束两个角色让它们在握手时正确对齐，约束角色的脚让它与地板对齐，等等
- `Attachments` 几乎所有的现代游戏引擎允许物体附着在其他物体上。最简单的，物体间的附着涉及约束物体 A 的骨架中的特定关节 $J_A$ 的位置和/或方向，使得它与物体 B 的骨架中的关节 $J_B$ 重合。
  - 附着通常为父子关系，当父关节的骨架移动时，调整子物体以满足约束；但是当子移动时，父关节的骨架通常不受影响
  - 有时在父关节与子关节间引入一个偏移量（`offset`）会很方便，例如枪的“握”关节与角色的“右手腕”关节间的约束，但这可能无法产生枪与手的正确对齐。一个解决方案是在其中一个骨架中引入一个特殊关节，例如给角色骨架添加一个“右枪”关节作为“右手腕”关节的儿子并正确放置，使枪的“握”关节与它对齐时，枪看起来被角色自然持有。这个方法最大的问题是增加了骨架关节的数量，通常不是一个可行的选择
  - `attach points` 附着点，特殊的关节，可以被动画流水线忽略，但仍可用于附着目的。可以在 Maya 中像常规的关节和定位器一样建模，但是许多游戏引擎使用更方便的方式定义附着点，例如作为 ASM 文本文件的一部分定义或通过动画创作工具的图形界面定义，这样做允许动画师聚焦于影响角色外观的关节，而控制附着的权力方便地交到了需要它的人手中：游戏设计师和工程师
- `Interobject Registration` 物体间注册
  - `Reference Locators` 参考定位器，Maya 中的特殊定位器。动画导出时参考定位器的位置和方向以相对于每个行动者的局部空间的坐标表示并保存到动画片段数据文件中，当这些动画片段在游戏中回放时，动画引擎可以查找片段中参考定位器的位置和方向，然后变换多个物体的原点，使得所有参考定位器在自然空间中重合（很像附着点，实际上可以实现为一个附着点），就像它们在原始 Maya 场景中一样对齐
  - `Finding the World-Space Reference Location` 一个重要的细节是如何决定参考定位器在自然空间中的位置和方向
    - 向其中一个物体请求参考定位器的位置，然后对齐其他物体
    - 独立于所有物体定义参考定位器的自然空间变换
- `Grabbing and Hand IK` 即使使用附着将两个物体连接，有时也会发现对齐在游戏中看起来并不完全正确，例如角色右手握步枪，左手支撑枪托，向各个方向瞄准武器时，我们可能会注意到在特定目标角度左手不再与枪托正确对齐。这种关节错位是由 LERP 混合引起的
  - 一个解决方案是使用 IK 纠正左手的位置。IK 系统的 API 通常采用在特定关节链和所需目标点上启用或禁用 IK 的请求的形式。
  - IK 通常很少启用和禁用，但是如果目标在移动，其在自然空间的位置必须保持每帧更新，所以底层动画流水线总是提供一些更新活动 IK 目标点的机制，例如启用 IK 的 API 可能允许多次调用，第一次调用启用 IK 链并设置目标点，后续调用更新目标点；另一种方法是将目标链接到游戏中的动态物体上，例如将 IK 目标定义为刚体的一个句柄或动画物体的一个关节
  - 大多数 IK 算法只解决关节的位置，所以可能需要编写额外的代码保证末端执行器的方向也与它的目标正确对齐
- `Motion Extraction and Foot IK` 滑动脚可以通过多种方式克服，其中最常见的是运动提取（`motion extraction`）和足部 IK
  - `Motion Extraction` 运动提取
    - 在 Maya 中，动画师让角色向前迈出一步，首先是左脚，然后是右脚，结果动画片段称为运动周期（`locomotion cycle`）。角色从 0 帧的初始位置开始移动到周期结束时的新位置，动画师注意确保角色的脚在移动时看起来接地且不会出现滑动。这个过程中角色的局部空间原点固定，当该动画循环播放时，可以看到角色向前迈出一步，然后回到它在动画第一帧的位置
    - 在游戏中使用该动画，必须保持角色的局部空间原点在任何时候在角色的质量中心下，这可以通过归零角色骨架根关节的向前平移完成。归零前的动画数据可以保存到特殊的“提取的运动”通道中，用于在游戏中移动角色的局部空间原点，使得角色像在 Maya 中创作时那样向前行走但它的局部空间原点一直跟随，允许动画的正确循环
    - 可以计算角色在游戏中移动的平均速度来决定动画的播放速率，这对于自然的步行周期是可行的，但是角色前行的速度不是常量，特别当角色在跛行时这一点尤为明显
  - `Foot IK` 足部 IK
    - 角色移动路径与动画师创作的路径完全匹配时，运动提取可以做得很好，但是真实游戏中角色会以不同于动画师创作的方式转向和移动，仍然会造成脚滑动
    - 可以使用 IK 纠正滑动脚，基本思想是分析动画以确定每只脚与地面完全接触的时间段。当一只脚接触地面时，记住它的自然空间位置，对于它保持在地面上的所有后续帧，使用 IK 调整腿部姿势以使它保持固定在合适位置。该技术需要大量迭代和微调，且一些自然的人类运动无法通过 IK 单独产生，例如通过增加步伐来转向
    - 动画的外观和角色的感觉之间存在很大的权衡，特别对于人控制的角色。让玩家角色控制系统感觉到响应和乐趣通常比让角色动画看起来完美更重要
    - 不要轻易将运动提取和足部 IK 添加到游戏中，要为大量试验和错误预估时间，并准备做出权衡，确保玩家角色看起来不错，感觉也很好
- `Other Kinds of Constraints` 其他种类的约束
  - `Look-at` 这是角色能够查看环境中的兴趣点的能力，角色看一个点可能用它的眼睛，或眼睛和头，或头和整个上半身的扭曲。查看约束有时使用 IK 或程序性关节偏移量实现，尽管使用加性混合通常可以生成更自然的外观
