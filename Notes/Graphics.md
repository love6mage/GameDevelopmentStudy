...
- `Effect Files` 一个着色器程序描述了一个渲染传递（`pass`）内应用的的操作，但是有些视觉效果需要多个渲染传递，且 PC 平台为了兼容性需要定义视觉效果的退却版本。将多个着色器程序连接起来作为一个完整的视觉效果，这样的文件格式称为效果文件
  ...
  - 通常采用的分层结构
    ...
    - 每个技术内定义多个传递，一个传递描述如何渲染单个全帧图像，通常包含一个着色器程序的引用，各种参数绑定和和可选的渲染状态设置
- `Antialiasing` 抗锯齿，使用离散的像素集合对平滑连续的图像进行采样会产生锯齿，术语抗锯齿描述任何降低这种视觉失真的技术。抗锯齿有很多方法，但基本所有的技术都是通过混合周边像素来软化三角形的边。
